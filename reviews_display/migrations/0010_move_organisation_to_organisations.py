...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for review in orm.Review.objects.all():
            # Add the org in the organisation Foreign Key into the organisations ManyToMany
            review.organisations.add(review.organisation)

    def backwards(self, orm):
        for review in orm.Review.objects.all():
            # Add the first org in the ManyToMany organisations into the organisation Foreign Key
            review.organisation = review.organisations.all()[0]
            review.save()
