...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        # Set a fake value for all the star ratings. This is only needed so
        # that we can run the next migration which will set the
        # unique_together constraint. The values set will be scrapped the next
        # time that the initial_data is loaded.
        star_rating = 0

        for rating in orm['reviews_submit.Answer'].objects.all():
            star_rating += 1
    ...
    def backwards(self, orm):
        ...
        # Change all the star_rating values back to NULL
        orm['reviews_submit.Answer'].objects.all().update(star_rating=None)
    ...
    models = {
        ...
        'reviews_submit.answer': {
            'Meta': {'ordering': "['star_rating']", 'object_name': 'Answer'},
