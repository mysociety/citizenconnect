...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        # change all the public_reporter_name_original to be same as public_reporter_name
        orm.Problem.objects.filter(public_reporter_name=True).update(public_reporter_name_original=True)

        # probably not needed, but best to be safe.
        orm.Problem.objects.filter(public_reporter_name=False).update(public_reporter_name_original=False)

    def backwards(self, orm):
        # set the all back to False which is the default
        orm.Problem.objects.all().update(public_reporter_name_original=False)
    ...
    models = {
        ...
        'issues.problem': {
            ...
            'publication_status': ('django.db.models.fields.IntegerField', [], {'default': '2'}),
            'reporter_email': ('django.db.models.fields.EmailField', [], {'max_length': '254'}),
