...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        db.create_index('issues_question', ['category'])

        # Adding index on 'Question', fields ['status']
        db.create_index('issues_question', ['status'])

        # Adding index on 'Problem', fields ['category']
        db.create_index('issues_problem', ['category'])

        # Adding index on 'Problem', fields ['status']
        db.create_index('issues_problem', ['status'])


    def backwards(self, orm):
        # Removing index on 'Problem', fields ['status']
        db.delete_index('issues_problem', ['status'])

        # Removing index on 'Problem', fields ['category']
        db.delete_index('issues_problem', ['category'])

        # Removing index on 'Question', fields ['status']
        db.delete_index('issues_question', ['status'])

        # Removing index on 'Question', fields ['category']
        db.delete_index('issues_question', ['category'])
